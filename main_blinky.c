...
#define mainQUEUE_IMU_TASK_PRIORITY		        ( tskIDLE_PRIORITY + 2 )
#define	mainQUEUE_TRANSCEIVE_TASK_PRIORITY		( tskIDLE_PRIORITY + 1 )

/* The rate at which data is sent to the queue.  The 200ms value is converted
to ticks using the portTICK_PERIOD_MS constant. */
#define mainQUEUE_SEND_FREQUENCY_MS			( 1000 / portTICK_PERIOD_MS )
#define mainSOME_DELAY         ( 200 / portTICK_PERIOD_MS )
...
static void prvTransceiverTask( void *pvParameters );
static void xferIMUTask( void *pvParameters );
...
//	xQueue = xQueueCreate( mainQUEUE_LENGTH, sizeof( uint32_t ) );
	...
	if( xQueue != NULL )
	{
		...
		file. */
		xTaskCreate( xferIMUTask,				/* The function that implements the task. */
					"imuTask", 								/* The text name assigned to the task - for debug only as it is not used by the kernel. */
					configMINIMAL_STACK_SIZE, 			/* The size of the stack to allocate to the task. */
					NULL, 								/* The parameter passed to the task - not used in this case. */
					mainQUEUE_IMU_TASK_PRIORITY, 	/* The priority assigned to the task. */
					NULL );								/* The task handle is not required, so NULL is passed. */

       // xTaskCreate( prvTransceiverTask,
                    // "transceiverTask",
                    // configMINIMAL_STACK_SIZE,
                    // NULL,
                    // mainQUEUE_TRANSCEIVE_TASK_PRIORITY,
                    // NULL );
...
static void xferIMUTask( void *pvParameters )
{
    ...
    for( ;; )
    {
        ...
        readIMU();
        vTaskDelayUntil( &xNextWakeTime, mainSOME_DELAY );
        writePWM();
        vTaskDelayUntil( &xNextWakeTime, mainSOME_DELAY );
    }
}
